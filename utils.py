...
def get_detection_data_np(img, model_outputs, class_names):
    """
    ...
    return boxes, scores, np.array(class_names)[classes.astype('int64')]
...
class DataGenerator(Sequence):
    ...
    def get_data(self, annotation_line):
        ...
        if self.augmentors is not None:
            img, boxes = self.augmentor(img, boxes, self.augmentors)
        
        img = cv2.resize(img, (w, h))
        image_data = np.array(img).astype(np.float32) / 255.
    ...
    def augmentor(self, image, boxes, seq): 
        ...
        for box in bbs:
            aug_boxes.append([box.x1, box.y1, box.x2, box.y2, box.label])
        
        return image, np.array(aug_boxes)
